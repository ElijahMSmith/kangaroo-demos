...
import { addToInventory, buyIceCream, getStock } from "../controllers/iceCream.js";
...
router.post("/stock", async (req, res) => {
	...
	const error = await buyIceCream(flavor, scoops);
	if (error !== "") {
...
router.patch("/stock", async (req, res) => {
	...
	const error = await addToInventory(flavor, scoops);
	if (error !== "") {
		res.status(400).json({ error });
		return;
	}
